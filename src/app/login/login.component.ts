...
export class LoginComponent {
  ...
  onSubmit() {
    ...
    this.authService.login(credentials).subscribe({
      next: () => {
        ...
        if (role === 'admin') {
          this.router.navigate(['core/admin/admin-dashboard']);
        } else if (role === 'instructor') {
          this.router.navigate(['core/instructor/instructor-dashboard']);
        } else if (role === 'user') {
          this.router.navigate(['core/user/user-dashboard']);
        } else {
  ...
  onResetPassword(): void {
    ...
    const confirmPassword =
      this.resetPasswordForm.get('confirmPassword')?.value || ''; // Ensure confirmPassword is a string
